...
class Ticket extends Conectar
{
    ...
    public function insert_ticket($usu_id, $cat_id,$cats_id, $tick_titulo, $tick_descrip,$prio_id)
    {
        ...
        parent::set_names();
        $sql = "INSERT INTO tm_ticket (tick_id,usu_id,cat_id,cats_id,tick_titulo,tick_descrip,tick_estado,fech_crea,usu_asig,fech_asig,prio_id,est) VALUES 
...
public function insert_ticketdetalle($tick_id,$usu_id,$tickd_descrip){
    ...
    public function update_ticket_asignacion($tick_id,$usu_asig){
        ...
        $sql_check = "SELECT COUNT(*) AS count FROM tm_notificacion WHERE tick_id = ? AND usu_id = ?";
        $sql_check = $conectar->prepare($sql_check);
        $sql_check->bindValue(1, $tick_id);
        $sql_check->bindValue(2, $usu_asig);
        $sql_check->execute();
        $result = $sql_check->fetch(PDO::FETCH_ASSOC);

        /* Insertar notificación solo si no existe previamente */
        if ($result['count'] == 0) {
